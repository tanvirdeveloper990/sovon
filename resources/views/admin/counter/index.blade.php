    ...
    <div class="max-w-7xl mx-auto">
        <div class="bg-white shadow-lg rounded-2xl overflow-hidden border border-gray-100">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-cyan-600 px-6 py-4 text-white">
                <h3 class="text-xl font-semibold tracking-wide">Counter List</h3>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-5 py-3 text-gray-800 font-medium">{{ $item->counter}}</td>
                            <td class="px-5 py-3 text-gray-800 font-medium">
                                @if($item->icon)
                                <img src="{{Storage::url($item->icon)}}" alt="empty" class="w-20 h-20">
                                @endif
                            </td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-5 py-3 text-center">
                                <div class="flex justify-center items-center gap-2">
                                    ...
                                    <form id="delete-form-{{ $item->id }}"
                                        action="{{ route('admin.counters.destroy', $item->id) }}" method="POST"
                                        class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>
