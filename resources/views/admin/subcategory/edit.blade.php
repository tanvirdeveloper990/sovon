...
<section class="p-5 bg-gray-100 min-h-screen">
    <div class="mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
        ...
        <!-- Form Body -->
        <div class="p-8">
            <form method="POST" action="{{ route('admin.subcategories.update', $data->id) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="file" name="image"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:border-cyan-500 focus:ring-0 outline-none">
                        @if($data->image)
                            <img src="{{ asset('storage/' . $data->image) }}" class="w-24 h-24 object-cover mt-2 rounded-lg border">
                        @endif
                    </div>
